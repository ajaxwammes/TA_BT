"""
Created on Mon Dec 14 14:29:47 2020
@author: mart.vos
...
class Rebalancer:
    ...
    def individual_check(self,begin_portfolio):
        ...
        value_portfolio_start = sum(begin_portfolio['Value_per_stock'])
        value_portfolio_now = sum(self.environment['Value_per_stock'])
        amount_reallocation = round(value_portfolio_start - value_portfolio_now, 2)
        amount_reallocation_ratio = amount_reallocation / value_portfolio_now

        if round(value_portfolio_start - value_portfolio_now,2) == 0:
            print('No rebalancing to be done')
        else:
            print('Rebalancing...')
            if amount_reallocation_ratio > RTL.individual_rebalance_threshold:
                ...
                list1 = self.list_incorp_analyst_ratings(no_of_trades)
                for j in list1:
    ...
    def capital_check(self,begin_portfolio):
        ...
        if rest_value != 0:
            ...
            self.environment['Value_per_stock'] = round(self.environment['Value_per_stock'],2)
        else:
    ...
    def list_incorp_analyst_ratings(self,no_of_trades):
    ...
    def run(self,begin_portfolio):
        self.weighted_allocation()
        self.individual_check(begin_portfolio)
        self.capital_check(begin_portfolio)
        self.get_trades(begin_portfolio)
        self.weighted_allocation()
